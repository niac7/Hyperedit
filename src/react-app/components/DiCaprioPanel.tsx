...
export default function DiCaprioPanel({
  ...
  const generateFromImage = async (videoPrompt: string, imageId: string) => {
    ...
    try {
      const response = await fetch(`/session/${sessionId}/generate-video`, {
        method: 'POST',
  ...
  const restyleVideo = async (stylePrompt: string, videoId: string) => {
    ...
    try {
      const response = await fetch(`/session/${sessionId}/restyle-video`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: stylePrompt,
  ...
  const removeBackground = async (videoId: string) => {
    ...
    try {
      const response = await fetch(`/session/${sessionId}/remove-video-bg`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          videoAssetId: videoId,
                    ...
                    {message.video && (
                      <div className="mt-3">
                        <div className="relative rounded-lg overflow-hidden bg-zinc-900">
                          <video
                            src={`${message.video.streamUrl}`}
                            controls
                            className="w-full h-auto"
                            poster={message.video.thumbnailUrl ? `${message.video.thumbnailUrl}` : undefined}
                          />
